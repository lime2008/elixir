...
// 下载更新文件
async function downloadLatestJsUpdate(url: string): Promise<{ success: boolean; content?: string; error?: string }> {
...
// 获取远程版本信息
async function getRemoteVersionInfo(): Promise<{ success: boolean; md5?: string; url?: string; error?: string }> {
...
// 主更新触发函数
async function updateAndExecuteLatestJs(): Promise<{ success: boolean; updated: boolean; executed: boolean; error?: string }> {
  try {
    log('====================================');
    log('开始执行更新和代码加载流程');
...
async function initApp() {
  ...
  // 测试功能可以在实际使用时手动调用，这里暂时移除测试代码

// 不需要导出，只需要挂载到window上

// logger测试函数 - 可以手动调用以验证日志功能
function testLogger() {
  console.log('开始测试ELIXIR.logger功能...');
  
  try {
    // 检查ELIXIR.logger是否可用
    if ((window as any).ELIXIR?.logger) {
      const logger = (window as any).ELIXIR.logger;
      ...
      console.log(`已记录的日志数量: ${logs.length}`);
      
      // 测试按级别获取日志
      const errorLogs = logger.getLogs(logger.LogLevel.ERROR);
      console.log(`错误日志数量: ${errorLogs.length}`);
      
      console.log('✓ ELIXIR.logger功能测试完成');
      return { success: true, totalLogs: logs.length, errorLogs: errorLogs.length };
    } else {
      console.error('✗ ELIXIR.logger未初始化');
      return { success: false, error: 'ELIXIR.logger未初始化' };
    }
  } catch (err) {
    console.error('✗ ELIXIR.logger测试失败:', err);
    return { success: false, error: err instanceof Error ? err.message : String(err) };
  }
}

// 导出测试函数供外部调用
(window as any).testELIXIRLogger = testLogger;

// 注意：取消下面这行的注释可以在应用加载时自动运行测试
// testLogger();